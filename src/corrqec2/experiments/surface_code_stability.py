...
from ._stability_builder.circuits import surface_code_stability_experiment_circuit
...
class SurfaceCodeStability(Experiment):
    ...
    def _build_circuit(self) -> stim.Circuit:
        return surface_code_stability_experiment_circuit(
            diam=self.distance,
            rounds=self.rounds,
            basis=self.basis,
        )
    ...
    def gen_split_circuits(self) -> list[stim.Circuit | tuple[int, stim.Circuit]]:
        ...
        for i, instr in enumerate(circuit[::-1]):
            if instr.name == "TICK" and i_tick_reverse is None:
                i_tick_reverse = len(circuit) - i - 1
                break
        ...
        circuit_final_round = circuit[i_block + 1 : i_tick_reverse + 1]
        circuit_final = circuit[i_tick_reverse + 1 :]

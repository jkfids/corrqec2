...
class Sampler:
    def __init__(
        self, experiment: Experiment, noise_model: NoiseModel, batch_size: int = 1
    ):
    ...
    def _sample_batch(self):
        error_matrix = self._noise_model.gen_error_matrix(
            self._experiment, self._sim.batch_size
        )
    ...
    def _sample_error_matrix(
        ...
        detection_events = (
            self._sim.get_detector_flips().transpose()
    ...
    def _apply_pauli_flips(
        self, error_masks: tuple[np.ndarray, np.ndarray, np.ndarray], i: int
    ):

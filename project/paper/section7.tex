...
We structure the analysis as follows. In \cref{sec:qca-details}, we present the microscopic QCA model, and its mapping, under operational system twirling, to an effective SPP with an exact PCA description. In \cref{sec:qca-stat-mech}, we characterise the resulting PCA as a statistical mechanical system, isolating how the spatial parameter controlling coherent bath interactions drives a critical-like transition in the bath dynamics. Finally, in \cref{sec:qca-qec-results}, we benchmark rotated surface code memory under this noise model and show a sharp breakdown of distance scaling in the pseudo-critical regime.
...
Let \(s_{t}(i) \in \{0,1\}\) denote the state of environment site \(i \in V\) at the beggining of cycle \(t\). The Pauli emission pattern during cycle \(t\) is denoted by the string \(x_t \in \{x_t(i)\}_{i \in V}\), where each \(x_t(i) \in \{I, X, Y, Z\}\) is sampled conditional on the updated bath configuration \(s_{t+1}(i)\) at the end of the cycle.
...
Before studying the noise properties of this model for QEC, we first analyse the PCA dynamics from \cref{sec:pca-details} as a statistical mechanical system. In particular, we isolate the effect of the spatial parameter \(\theta\), which controls the strength of the nonlinear PCA update via kernel \(p = \sin^2(k\theta)\), on the emergent correlation structure and steady-state properties of the system.
...
\begin{figure}[t]
    ...
    We fix the storm parameters to \(a=10^{-4}\) and \(b=0.5\), and sweep the spatial coupling parameter \(\theta\), which controls the strength of coherent bath interactions. Data are shown for lattice corresponding to rotated surface code patches of distances \(d=9, 11, 13, 15\) (number of sites \(N_d = 2 d^2 - 1\)). As a function of \(\theta\), we plot (a) the mean activity (spin) density \(\langle \eta_t\rangle_t\), where \(\eta_t \coloneq N_d^{-1}\sum_{i\in V} s_t(i)\); (b) the scaled variance \(N_d\,\mathrm{Var}(\eta_t)\); and (c) the correlation time \(\xi_\eta\) extracted by fitting the normalised autocorrelation \(C(\tau)\propto e^{-\tau/\xi_\eta}\). The vertical dashed line marks the estimated pseudo-critical threshold \(\theta_{\mathrm{th}}\approx 0.39\pi\).}
...
We now conclude by discussing future directions and extensions unlocked by the SPP framework.

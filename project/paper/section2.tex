...
\begin{itemize}
    ...
    \item Citations (examples of non-Markovian environments in QEC)
...
\end{equation}
with \(p_\alpha \geq 0\) and each \(\Lambda_j^\alpha\) is the Choi representation of a CPTP channel. Processes that can be written in this form are labelled \emph{process separable}, whose temporal correlations are classically simulable. Conversely, a process is fully \emph{Markovian}---exhibiting neither quantum nor classical temporal correlations---if and only if its Choi operator factorises across all time steps,
\begin{equation}
...
% The process tensor compactly encapsulates all uncontrollable system-environment interactions. Operationally, non-Markovianity can be regarded as the ignorance of a larger Markovian dynamics on the joint system-environment Hilbert space. 
...
\subsection{Effective environment dephasing}\label{app:env-dephasing}
...
\end{equation}
so that \(\Delta_E = \Delta_B \circ \Delta_R = \Delta_R \circ \Delta_B\).

\subsection{PCA kernel}\label{app:pca-kernel}
...
\end{equation}
This expression allows us to insert \(\Delta_B\) immediately after the first half-step \(\mathcal{Q}_{R \to B}\). Crucially, this allows us to show that the intermediate coherences in \(B\) generated by the first half-step do not affect the final distribution, and that the two QCA half-steps can be treated as separate PCA updates.
...
\subsection{System Pauli emission}\label{app:system-pauli-emission}
...
\end{equation}
Using orthogonality of Pauli operators under the Hilbert-Schmidt inner product, we obtain the system Pauli channel
\begin{equation}
...
\subsection{Summary of mapping}\label{app:summary-mapping}
We now summarise the full mapping from the microscopic QCA noise model to the effective PCA HMM induced by the system Pauli twirl. We iterate through the PCA HMM steps as defined in \cref{sec:pca-details}, and justify each step in reference to the assumptions and derivations in \cref{app:assumptions,app:env-dephasing,app:pca-kernel,app:system-pauli-emission}.
...
\begin{enumerate}
    ...
    Assumed in~(A2). The storm channel \(\mathcal{S}^E\) is environment-only and preserves diagonal \(\rho_E\), so it is simulated exactly as a classical update on \(s_t\) and does not affect the subsequent reduction.
